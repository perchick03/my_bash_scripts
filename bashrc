...
# Parse branch name from git. This is used in the prompt.
parse_git_branch() {
...
# Based on https://raw.githubusercontent.com/ChrisTitusTech/scripts/master/fancy-bash-promt.sh
...
bash_prompt() {
    ...
    local PROMT_GIT=$"$TEXT_FORMAT_3\$(parse_git_branch)"
	...
	*)
    ...
    export PS1="$TITLEBAR\n\`if [ \$? = 0 ]; then echo '$TOXIC_GREEN_BOLD'^_^'$RS'; else echo '$RED_BOLD'O_O'$RS'; fi\`${SEPARATOR_1}${PROMT_PWD}${SEPARATOR_2}${PROMT_GIT}$RS${SEPARATOR_3}${PROMT_INPUT}"
